...
extension LayoutSwizzlable where Self: TextAlignmented & Taggable {
    func handleSwitching(forceSwitchingRegardlessOfTag: Bool) {
        if tag < MOLH.shared.maximumLocalizableTag + 1 || forceSwitchingRegardlessOfTag, textAlignment != .center {
...
open class MOLH {
    ...
    open class func setLanguageTo(_ language: MOLHLanguages) {
        MOLHLanguage.setAppleLAnguageTo(language.rawValue)
...
extension Bundle {
    @objc func specialLocalizedStringForKey(_ key: String, value: String?, table tableName: String?) -> String {
        ...
        let translate = { (tableName: String?) -> String in
            ...
            let languageWithoutLocale = MOLHLanguage.currentAppleLanguage().rawValue // without locale
            var bundle = Bundle()
...
public extension UIImage {
    func flippedImage() -> UIImage? {
        if let _cgImag = cgImage {
            let flippedimg = UIImage(cgImage: _cgImag, scale: scale, orientation: UIImage.Orientation.upMirrored)
            return flippedimg
        }
        return nil
    }
...
extension UIViewController {
    @objc func mirroringviewDidLoad() {
        ...
        if UIApplication.shared.userInterfaceLayoutDirection == .rightToLeft {
            loopThroughSubViewAndFlipTheImageIfItsNeeded(view.subviews)
        }
...
extension UIControl {
    internal func handleControlSwitching(forceSwitchingRegardlessOfTag: Bool) {
        if tag < MOLH.shared.maximumLocalizableTag + 1 || forceSwitchingRegardlessOfTag {
            if MOLHLanguage.isRTLLanguage() {
...
extension UITextField: TextViewType {
    override public func cstmlayoutSubviews() {
        cstmlayoutSubviews()
